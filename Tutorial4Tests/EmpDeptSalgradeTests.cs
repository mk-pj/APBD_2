...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = (from e in emps
                                             ...
                                             select new { d.DName, e.EName });

        // var result = 
        //     emps.Join(depts, e => e.DeptNo, e => e.DeptNo, (emp, dept) => new { dept.DName, emp.EName });
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        //     .Where(g => e.Sal >= g.Losal && e.Sal <= g.Hisal), (e, s) => new { e.EName, s.Grade });
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = (from e in emps
                               where (
                                   ...
                                   select emp.Sal).Average() < e.Sal
                               select e.EName).ToList();

        // var result = emps.Where(e => 
        //         e.Sal > emps.Where(emp => e.DeptNo == emp.DeptNo)
        //                     .Average(emp => emp.Sal))
        //     .Select(e => e.EName)
        //     .ToList();
